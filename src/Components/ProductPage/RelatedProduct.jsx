...
function RelatedProduct() {
  ...
  return (
    <>
      {relatedItems?.[0].title === "You might also like" && (
        <div className="xl:w-[85%] w-full mx-auto px-2">
          <h2 className="text-[#333] font-bold text-3xl mb-4">
            {relatedItems[0].title}
          </h2>
          <div className="grid  grid-cols-[repeat(auto-fill,minmax(100px,1fr))] justify-items-center gap-4">
            {relatedItems[0]?.items.map((item, i) => (
              ...
              >
                <Link to={`/Product/${item.product_id}`}>
                  <div className="w-full h-[100px] bg-white flex justify-center items-center mb-2">
                    <img
                      className="object-cover block w-fit h-full"
                      src={item.image}
                      alt=""
                    />
                  </div>
                </Link>
                <p className="text-[#0B8500] text-xl text-center font-bold">
                  {item.currency}
                  {item.price}
                </p>
              </div>
      ...
      {relatedItems?.[0].title === "Related items" && (
        <div className="xl:w-[85%] w-full mx-auto px-2">
          <h2 className="text-[#333] font-bold text-3xl mb-4">
            {relatedItems[0].title}
          </h2>
